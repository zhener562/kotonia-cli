...
pub enum Event {
    ...
    /// Code engine emits this today; consumers should render it as a partial
    /// text chunk and avoid replaying the same content under `Final`.
    Text {
        text: String,
    },
    Bash {
...
impl Agent {
    pub fn new(workspace_root: &Path, provider: Provider, config: AgentConfig) -> Self {
        ...
        let tool_catalog = if native_mode {
            build_tool_catalog()
        } else {
            Vec::new()
        };
    ...
    async fn run_turn_native(
        ...
        for iter in 1..=self.config.max_iterations {
            ...
            for (id, name, input) in tool_uses {
                let result = self.dispatch_tool(&id, &name, &input, approval, sink).await;
                match result {
                    Ok((content, is_error)) => {
            ...
            // Multimodal tools (inspect_image today) queue a follow-up user
            // message carrying the image content. Drain right after the tool
            // message so message order is:
    ...
    /// One LLM call on the native path. Sends `tool_choice: "required"` (an
    /// action every turn — `final_answer` is how the model finishes); if the
    /// backend rejects the parameter, demote to `auto` for the rest of the
    ...
    async fn dispatch_tool(
        ...
        match name {
            ...
            "fetch_url" => {
                ...
                let max_chars = input.get("max_chars").and_then(|v| v.as_u64()).unwrap_or(0);
    ...
    /// Resolve `path` against the agent's workspace cwd (same policy as the
    /// host `bash` tool), read the image, and return `(media_type, base64,
    /// size_bytes)`. Refuses large files so one asset cannot bloat context.
    fn read_image_for_inspection(&self, path: &str) -> Result<(String, String, u64), String> {
        use base64::Engine;
...
/// Extract the answer text from a `final_answer` tool call. Falls back to the
/// assistant's prose content when the `answer` argument is missing or blank.
...
/// Detect an API rejection of `tool_choice: "required"` and allow a one-time
/// demotion to `auto`.
...
fn build_tool_catalog() -> Vec<AiTool> {
    vec![
        ...
        AiTool {
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    "answer": {
                        "type": "string",
                        "description": "The final answer to show the operator."
                    }
                },
                "required": ["answer"]
            }),
        ...
        AiTool {
            ...
            description: "Attach an image file from the workspace to your next \
                          reasoning turn so you can inspect it visually. Use \
                          this after generating or editing images before \
                          claiming success. Supports png/jpg/jpeg/webp/gif up \
                          to 10 MB."
                .to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Path to an image file, relative to the workspace or absolute."
                    }
                },
                "required": ["path"]
            }),
...
mod tests {
    ...
    fn final_answer_prefers_answer_argument() {
        let input = json!({"answer": "done"});
        assert_eq!(final_answer_text(&input, "prose"), Some("done".to_string()));
    }
    ...
    fn tool_catalog_includes_final_answer() {
        ...
        assert!(final_tool.description.contains("Finish the task"));
    }
